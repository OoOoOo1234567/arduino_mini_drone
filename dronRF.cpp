...
void RXnrf::loop() {
    if (radio.available()) {
        ...
        Serial.print(" | Y: "); Serial.println(data.yaw);

        Serial.println("////////////////"); // שיהיה נוח 
        delay(100); 
        ...
        TXwantthrottle = constrain(TXwantthrottle, 0, 255);
        TXwantroll = constrain(TXwantroll, -127, 127);
        TXwantpitch = constrain(TXwantpitch, -127, 127);
        TXwantyaw = constrain(TXwantyaw, -127, 127);
